...
''' Initialization Phase:
    Here the weights, the nadir point and the initial nondominated
     solution P are generated. In addition, the prioblem is defined'''
...
LIP=np.empty((0, problem.num_of_objectives), float) # List of previous reference points
rho=0.1 # Parameter in ASF
Generations = 50 # Number of generations in WASF-GA to get the initial well-spread set of solutions

for iter in range(NI):
############### ------------- Start generating solutions for F-NAUTILUS ----------------------- ################
     ...
     # The DM selects a point (here a point reference_point is selected randomly)
     ...
     LIP=np.vstack((LIP,zh_prev)) # update the list of previous points
